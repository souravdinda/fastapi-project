...
async def root():
    """Serve a Hello World HTML page, optionally fetching from API Gateway."""
...
<body>
  <div class="card">
    <h1>{api_message}</h1>
    <div class="badge">✓ Live from EC2 via ALB</div>
    <div class="stack">
      <span class="tag">FastAPI</span>
      <span class="tag">EC2</span>
      <span class="tag">ALB · port 80</span>
...
async def hello_json():
    """JSON endpoint that proxies the Lambda via API Gateway."""
    if API_GATEWAY_URL:
        try:
            async with httpx.AsyncClient(timeout=5) as client:
                resp = await client.get(API_GATEWAY_URL)
